...
onMounted(() => {
  ...
  if (!uid.value || !token.value) {
    errorMsg.value = 'Enlace de verificación de cuenta inválido o incompleto.'
  }
})
...
const handleActivateAccount = async () => {
  ...
  try {
    const res = await api('/cuentas/verificacion/confirmar/', {
      method: 'POST',
      body: JSON.stringify({
        uid: uid.value,
        token: token.value,
    ...
    if (res.ok) {
      successMsg.value = 'Cuenta verificada e inicializada correctamente. Redirigiendo...'
      ...
      errorMsg.value = errorData.password_nueva?.[0] || errorData.uid?.[0] || 'Error al verificar la cuenta. Es posible que el enlace haya expirado o ya esté activa.'
    }
  } catch {
    errorMsg.value = 'Error de conexión al activar la cuenta.'
  } finally {
  ...
  <div class="min-h-screen w-full flex items-center justify-center bg-mako-950 text-mako-900 font-sans overflow-hidden relative">
    ...
    >
      <div class="text-center mb-8 flex flex-col items-center justify-center">
        ...
        <h1 class="text-2xl font-bold tracking-tight text-mako-900 dark:text-white">Activar Cuenta</h1>
        <p class="text-[10px] text-mako-500 dark:text-mako-400 mt-1.5 uppercase tracking-widest font-bold">
          Plataforma de Monitoreo IoT
        </p>
      </div>

      <p class="text-xs text-mako-600 dark:text-mako-300 text-center mb-6">
        Estás a un paso de activar tu cuenta. Configura una contraseña para comenzar.
      </p>

      <form @submit.prevent="handleActivateAccount" class="space-y-5">
        <div>
          <label class="block text-xs uppercase font-bold tracking-wider text-mako-500 dark:text-mako-300 mb-2"
            >Establecer Contraseña</label
          >
          <input
            v-model="password"
